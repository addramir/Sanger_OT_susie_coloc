...
CARMA_fixed_sigma<-function(z.list,ld.list,w.list=NULL,lambda.list=NULL,output.labels='.',label.list=NULL,
  ...
  Module.Cauchy.Shotgun<-function(z,ld.matrix,Max.Model.Dim=1e+4,input.S=NULL,lambda,label,
    ...
    #Postertiot Exclusion Probabilities
    PEP.func<-function(likeli,model.space){
      infi.index<-which(is.infinite(likeli))
      if(length(infi.index)!=0){
        likeli<-likeli[-infi.index]
        model.space<-model.space[-infi.index,]
      ...
      result.prob<-rep(NA,p)
      for(i in 1:p){
        result.prob[i]<-sum(exp(aa[which(model.space[,i]==0)]))/prob.sum
        #result.prob[i]<-sum(exp(aa[which(model.space[,i]==1)]))/sum(exp(aa[which(model.space[,i]==0)]))
      }
      return(result.prob)
    }
    ...
    for(l in 1:inner.all.iter){
      ...
      if(num.causal==1){
        ...
        PEP=PEP.func(result.B.list[[1]],result.B.list[[2]])
      }
    ...
    return(list(result.B.list,C.list,result.prob,conditional.S.list,prob.list,PEP))
  }
  ######## Burning step###########
